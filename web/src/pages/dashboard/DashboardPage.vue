            ...
            <el-table :data="dashboard.budget_alerts" size="small" v-if="dashboard.budget_alerts?.length">
              ...
              <el-table-column prop="usage_rate" :label="t('budget.usageRate')">
                <template #default="{ row }">
                  <el-progress :percentage="Math.min(Math.round(row.usage_rate * 100), 100)" :stroke-width="18" :text-inside="true" :status="row.status === 'overspent' ? 'exception' : row.status === 'warning' ? 'warning' : undefined" />
                </template>
              </el-table-column>
            </el-table>
