...
erDiagram
    ...
    BudgetCategory {
        uint64 budget_id PK_FK
        uint64 category_id PK_FK
    }
    ...
    %% === 用户核心关系 ===
    User ||--o{ Account : "has_many"
    User ||--o{ Transaction : "has_many"
    User ||--o{ Category : "has_many"
    User ||--o{ Tag : "has_many"
    User ||--o{ Budget : "has_many"
    User ||--o{ PiggyBank : "has_many"
    User ||--o{ RecurringTransaction : "has_many"
    User ||--o{ RuleGroup : "has_many"
    User ||--o{ Rule : "has_many"
    User ||--o{ Webhook : "has_many"
    User ||--o{ Preference : "has_many"
    User ||--o{ Attachment : "has_many"
    User ||--o{ BackupCode : "has_many"
    User ||--o{ ObjectGroup : "has_many"

    %% === 账户与货币 ===
    Currency ||--o{ Account : "has_many"
    Currency ||--o{ ExchangeRate : "from_currency"
    Currency ||--o{ ExchangeRate : "to_currency"

    %% === 交易核心关系 ===
    Account ||--o{ Transaction : "source (SourceID)"
    Account ||--o{ Transaction : "destination (DestinationID)"
    Transaction ||--o| Category : "belongs_to (CategoryID)"
    Transaction }|--|{ Tag : "many2many (transaction_tags)"
    Transaction ||--o{ Transaction : "splits (ParentID)"
    Transaction ||--o| RecurringTransaction : "belongs_to (RecurringID)"

    %% === 分类与标签自引用 ===
    Category ||--o{ Category : "children (ParentID)"
    Tag ||--o{ Tag : "children (ParentID)"

    %% === 预算关系 ===
    Budget }|--|{ Category : "many2many (budget_categories)"
    Budget ||--o{ BudgetHistory : "has_many"

    %% === 储蓄罐关系 ===
    PiggyBank ||--o| Account : "belongs_to (AccountID)"
    PiggyBank ||--o{ PiggyEvent : "has_many"
    PiggyEvent ||--o| Transaction : "belongs_to (TransactionID)"

    %% === 循环交易关系 ===
    RecurringTransaction ||--o| Account : "source (SourceID)"
    RecurringTransaction ||--o| Account : "destination (DestinationID)"
    RecurringTransaction ||--o| Category : "belongs_to (CategoryID)"
    RecurringTransaction ||--o{ RecurringTransactionLog : "has_many"
    RecurringTransactionLog ||--o| Transaction : "belongs_to"

    %% === 规则引擎关系 ===
    RuleGroup ||--o{ Rule : "has_many"
    Rule ||--o{ RuleCondition : "has_many"
    Rule ||--o{ RuleAction : "has_many"

    %% === 交易关联与对账 ===
    Transaction ||--o{ TransactionJournalLink : "source"
    Transaction ||--o{ TransactionJournalLink : "linked"
    Account ||--o{ TransactionReconciliation : "has_many"
    TransactionReconciliation ||--o{ ReconciliationEntry : "has_many"
    Transaction ||--o{ ReconciliationEntry : "has_many"

    %% === Webhook 关系 ===
    Webhook ||--o{ WebhookMessage : "has_many"
    Webhook ||--o{ WebhookDelivery : "has_many"

    %% === 多态关系（虚线表示） ===
    Attachment ..> Transaction : "attachable"
    Attachment ..> Account : "attachable"
    Attachment ..> PiggyBank : "attachable"
    ObjectGroup ..> Account : "groupable"
    ObjectGroup ..> Category : "groupable"
```
