...
func NewRuleRepository(readDB, writeDB *gorm.DB) *RuleRepository {
	return &RuleRepository{readDB: readDB, writeDB: writeDB}
}
...
func (r *RuleRepository) GetEnabledRules(userID uint64) ([]model.Rule, error) {
	var rules []model.Rule
	if err := r.readDB.Where("user_id = ? AND is_enabled = ?", userID, true).
		Preload("Conditions").
		Preload("Actions").
		Order("priority ASC, id ASC").Find(&rules).Error; err != nil {
		return nil, err
...
func (r *RuleRepository) GetEnabledRulesByGroupID(groupID, userID uint64) ([]model.Rule, error) {
	var rules []model.Rule
	if err := r.readDB.Where("group_id = ? AND user_id = ? AND is_enabled = ?", groupID, userID, true).
		Preload("Conditions").
		Preload("Actions").
		Order("priority ASC, id ASC").Find(&rules).Error; err != nil {
		return nil, err
