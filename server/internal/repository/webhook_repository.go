...
func NewWebhookRepository(readDB, writeDB *gorm.DB) *WebhookRepository {
	return &WebhookRepository{readDB: readDB, writeDB: writeDB}
}

func (r *WebhookRepository) Create(webhook *model.Webhook) error {
	return r.writeDB.Create(webhook).Error
}

func (r *WebhookRepository) GetByID(id, userID uint64) (*model.Webhook, error) {
	var webhook model.Webhook
	if err := r.readDB.Where("id = ? AND user_id = ?", id, userID).First(&webhook).Error; err != nil {
		return nil, err
	}
	return &webhook, nil
}
...
func (r *WebhookRepository) List(userID uint64) ([]model.Webhook, error) {
	...
	if err := r.readDB.Where("user_id = ?", userID).Order("id ASC").Find(&webhooks).Error; err != nil {
		return nil, err
	}
	return webhooks, nil
}
...
func (r *WebhookRepository) Update(webhook *model.Webhook) error {
	return r.writeDB.Save(webhook).Error
}
...
func (r *WebhookRepository) GetActiveByTrigger(userID uint64, trigger model.WebhookTrigger) ([]model.Webhook, error) {
	var webhooks []model.Webhook
	if err := r.readDB.Where("user_id = ? AND trigger = ? AND is_active = ?", userID, trigger, true).
